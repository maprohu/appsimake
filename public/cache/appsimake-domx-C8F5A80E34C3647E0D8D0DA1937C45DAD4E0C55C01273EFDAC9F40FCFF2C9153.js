...
this['appsimake-domx'] = function (_, Kotlin, $module$appsimake_commonshr, $module$appsimake_common) {
  ...
  function clickEventSeq$lambda(closure$seq, closure$fn) {
    return function (it) {
      it.preventDefault();
      var tmp$;
      closure$seq.plusAssign_wii6vi$(closure$fn(Kotlin.isType(tmp$ = it, MouseEvent) ? tmp$ : throwCCE()));
      return Unit;
    };
  }
  function clickEventSeq($receiver, fn) {
    var seq = KillableSeq_init();
    $receiver.onclick = clickEventSeq$lambda(seq, fn);
    return seq;
  }
  ...
  NodeExt.prototype.updateChildren = function () {
    ...
    var firstUnknownChild = (tmp$ = lastKnownVisibleChildExt != null ? lastKnownVisibleChildExt.owner : null) != null ? tmp$ : this.owner.firstChild;
    var tmp$_1;
    tmp$_1 = nextSeq(firstUnknownChild).iterator();
  ...
  function rxDisplayed_0($receiver, rxv) {
    ...
    ensureNotNull(parentNodeExt.updateChildren());
  ...
  package$domx.clickEventSeq_xa4vqr$ = clickEventSeq;
